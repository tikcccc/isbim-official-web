...
**Êñá‰ª∂ËØ¥Êòé:** Êú¨Êñá‰ª∂ÂÆö‰πâ‰ª£Á†ÅÈ£éÊ†ºËßÑÂàô„ÄÅÂëΩÂêçÁ∫¶ÂÆö„ÄÅÂèçÊ®°ÂºèÂíåÊúÄ‰Ω≥ÂÆûË∑µ„ÄÇÂΩìÂª∫Á´ãÊñ∞ÁöÑÁºñÁ†ÅÊ®°Âºè(Âä®ÁîªÊó∂Â∫è„ÄÅÁªÑ‰ª∂ÁªìÊû?„ÄÅÂÆö‰πâÊñ∞Â∫ìÁöÑÂØºÂÖ•/ÂØºÂá∫ËßÑÂàô„ÄÅÊ∑ªÂä?TypeScript Á±ªÂûãÂÆâÂÖ®Ë¶ÅÊ±ÇÊàñËÆæÁΩÆÂä®ÁîªÂçèË∞ÉËßÑÂàôÊó∂ÈúÄË¶ÅÊõ¥Êñ∞Ê≠§Êñá‰ª∂„Ä?

**Êõ¥Êñ∞ÂéüÂàô:**
- ‰ΩøÁî®ÁÆÄÊ¥ÅÁöÑË¶ÅÁÇπÂíå‰ª£Á†ÅÁâáÊÆ?ÈÅøÂÖçÈïøÁØáËß£Èáä
- ËßÑÂàôÁÆÄÊòéÊâºË¶?ÂÖ≥ÈîÆÊó∂Âàª‰ΩøÁî®
- Âà†Èô§Â∑≤ËøáÊó∂Êàñ‰∏çÂÜçÈÄÇÁî®ÁöÑËßÑÂà?
...
- ImagesÂÑ™ÂÖà‰ΩøÁî®Êú¨Âú∞ `/public` Ë≥áÊ∫ê„Ä?
- ÂãïÊÖãÂÖßÂÆπÈ†ÅÔºàNewsroom„ÄÅCareersÔºâ‰ΩøÁî?Sanity ÁÆ°ÁêÜË≥áÊñôÔºõÂÖ∂‰ªñÈ†ÅÈù¢‰∏ç‰æùË≥¥ Sanity„Ä?
...
## Styling & Tokens
- Design tokens Œª”⁄ `src/styles/*-design-tokens.css` + `design-tokens.ts`£ª≤ª“™÷ÿ∏¥∂®“Â breakpoints/z-index/colors/fonts°£“≥√Ê/◊Èº˛◊÷ÃÂ÷ª”√∂‘”¶ token£®home/product/aboutus/services/contact/newsroom/layout£©£¨±‹√‚‘Ÿ–¥ `font-sans/font-mono`°£
- Layout µº∫Ω/“≥Ω≈/Topbar/Menu  π”√ `layout-design-tokens.css` (AllianceNo2 ±ÍÃ‚/AllianceNo1 ¡¥Ω”/±Í«©)£ª“≥√Ê token ∏∫‘±æ“≥◊÷∫≈/––∏ﬂ£®±£≥÷œ÷”–¥Û–°£©°£
- Spacing/shape/motion£∫Home/Product/About/Contact/Newsroom “—Ã·π© container/section/stack/grid°¢radius/shadow°¢motion (duration/easing/stagger) token£ªΩ˚÷π‘Ÿ–¥¡„…¢ `pt-28`/`gap-12`/`rounded-lg`/`duration-300`£¨∏ƒ”√∂‘”¶ utility£®»Á `home-section-padding`°¢`product-radius-card`°¢`about-section-padding`°¢`contact-form-grid`°¢`newsroom-section`£©°£
- ∂Ø–ß£∫Õ≥“ª”√»´æ÷ `animation-design-tokens.css` + “≥º∂ motion token£ªtypewriter/GSAP/Framer ≤Œ ˝∂¡ token£¨±‹√‚”≤±‡¬Î easing/ ±≥§°£
- Tailwind v4 utilities£ªshimmer utility »‘‘⁄ `globals.css`£®Services/Products hero£©°£
...
- **CRITICAL i18n Rule**:
  - ‚ù?NEVER call m.*() in Server Component and pass as props (pre-renders as static strings ‚Ü?locale mismatch)
  - ‚ú?ALWAYS create dedicated `{product}-client.tsx` marked `"use client"` with all m.*() calls inside
  - Example: `jarvis-pay/page.tsx` (Server) ‚Ü?`jarvis-pay/jarvis-pay-client.tsx` (Client with m.*())
...
- **Critical keywords**: All pages MUST include isBIM + Hong Kong/È¶ôÊ∏Ø + dual identity (AI + Construction tech). Generators enforce this automatically via `composeKeywords()`.
...
- **Design tokens**: Uses `contact-design-tokens.css` with product template gradient system (purple‚Üícyan: `#9881F3‚Ü?13C9BA`).
...
- **SEO**: Use `generateNewsroomPageSEO()` for list page; build detail page metadata from `NEWS_METADATA_QUERY` with fallbacks (seo.metaTitle ‚Ü?title, seo.metaDescription ‚Ü?subtitle/excerpt).
